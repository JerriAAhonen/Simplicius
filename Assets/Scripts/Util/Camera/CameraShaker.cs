// Taken from
// https://github.com/andersonaddo/EZ-Camera-Shake-Unity/blob/master/Assets/EZ%20Camera%20Shake/Scripts/CameraShaker.cs
...
namespace EZCameraShake
{
	...
	public class CameraShaker : MonoBehaviour
	{
		...
		private static readonly Dictionary<string, CameraShaker> instanceList = new();
		...
		private void Update()
		{
			...
			for (int i = 0; i < cameraShakeInstances.Count; i++)
			{
				...
				{
					posAddShake += CameraUtil.MultiplyVectors(c.UpdateShake(), c.PositionInfluence);
					rotAddShake += CameraUtil.MultiplyVectors(c.UpdateShake(), c.RotationInfluence);
				}
			}
		...
		{
			if (instanceList.TryGetValue(name, out var c))
				return c;
